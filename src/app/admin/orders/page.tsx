        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
            ...
            <div className="space-y-6">
              ...
              {toFulfill.length === 0 && (
                <div className="bg-white border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center">
                  <p className="text-gray-400 font-medium italic">No pending orders. {"You're all caught up!"}</p>
                </div>
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="space-y-6">
              ...
              {completed.length === 0 && (
                <div className="bg-white border-2 border-dashed border-gray-200 rounded-3xl p-12 text-center">
                  <p className="text-gray-400 font-medium italic">No completed orders yet.</p>
                </div>
              )}
            </div>
          </section>
...
function OrderCard({ order, showAction = false }: { order: OrderWithVenue; showAction?: boolean }) {
  ...
  return (
    <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden transition-all hover:shadow-md">
      <div className="p-6 md:p-8">
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <h3 className="font-bold text-gray-900 text-lg break-all">{order.customer_email}</h3>
            <p className="text-[10px] text-gray-400 font-mono mt-1 uppercase">ID: {order.id.slice(0, 8)}</p>
          </div>
          <div className="text-right">
            <p className="font-black text-2xl text-gray-900">{formatPrice(order.total_amount)}</p>
        ...
        {!isPickup && order.shipping_address && (
          <div className="bg-indigo-50/50 rounded-2xl p-4 text-sm text-indigo-900 mb-6 border border-indigo-100/50">
            <div className="flex items-center gap-2 font-black text-indigo-800 uppercase tracking-widest text-[10px] mb-2">
        ...
        {isPickup && (
          <div className="bg-blue-50/50 rounded-2xl p-4 text-sm text-blue-900 mb-6 border border-blue-100/50 flex items-center gap-3">
             ...
             <p className="font-bold text-blue-800">In-Store Pickup</p>
          </div>
        )}
